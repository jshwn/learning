...
즉시 쓰기는 쓰기 실패가 발생하면 다음 계층 메모리에도 바로 쓰는 방식이기 때문에 "적중 확인과 쓰기"를 동시에 수행하도록 회로를 설계할 수 있다. 즉, 적중 확인과 쓰기가 1사이클 안에 일어난다.
하지만 즉시 쓰기는 전체 프로그램에서 메모리 적재 명령어가 많으면 매우 비효율적이게 된다.
왜냐하면 캐시에도 쓰고 메모리에 값을 쓸 때까지 프로세서가 기다려야 하기 때문이다.
물론 메모리 접근 시간이 길어질수록 이 비효율성은 엄청나게 증가한다.
...
즉시 쓰기와 달리 나중 쓰기는 적중 여부를 확인한 뒤에야 쓸 수 있다.
그래서 나중 쓰기는 두 사이클이 걸리게 하거나, 저장 버퍼를 파이프라인화해서 거의 1사이클에 근접하게 할 수 있다.

##  쓰기 실패에 대한 처리
즉시 쓰기 캐시의 실패: 쓰기 할당(write allocate), 쓰기 비할당(no write allocate)

*   쓰기 할당: 전체 블록을 메모리에서 캐시로 불러온 후, 쓰기 실패를 발생시킨 워드만 덮어 쓴다.
*   쓰기 비할당: 메모리에 있는 블록의 해당부분만 갱신하고, 캐시에는 쓰지 않는 방식

### 사견
출처: https://qr.ae/pv4iUG
쉽게 말하면 쓰기 실패가 발생했을 때 갱신 내용을 해당 캐시에만 반영하느냐, 해당 캐시가 가리키는 메모리에만 반영하느냐의 차이이다.

만약 쓰기 실패가 발생한 블록이 축출되기 전에 프로세서가 해당 데이터를 읽어야 한다면 쓰기 할당을, 프로세서가 다시 읽을 일이 없다면 쓰기 비할당이 적절할 것이다.

쓰기 비할당은 IO 버퍼를 채울 때, 쓰기 할당은 함수 인자의 값을 참조할 때 적절할 것이다.
...
일반적인 RAM이 주소를 주면 워드를 반환하는 것과 달리,
CAM은 데이터를 주면 그 값을 확인하고 일치하는 행의 인덱스를 반환한다.
CAM은 SRAM 비교기를 사용한 구현기보다 더 높은 집합 연관도의 캐시를 구현할 수 있다.
다만 CAM의 크기와 전력 소모 문제로 2020년 8-way 이상의 집합 연관 캐시부터 CAM을 사용한다.
